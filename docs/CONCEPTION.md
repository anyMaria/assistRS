...
> **Statut** : en cours de validation avec Ana. Les éléments marqués 💡 sont des suggestions
> à accepter ou retirer. Une fois validé, ce document sert de base au guideline
> d'exécution détaillé destiné au modèle qui codera.
...
**Transversal** : recherche globale ⌘K, paramètres (vues, couleurs conditionnelles,
mappings CSV, budget API), PWA iPhone (phase ultérieure).
...
3. **Rapport mensuel PDF par marque** : page de garde aux couleurs de la marque, KPI du
   ...
   recommandations (section IA optionnelle). Généré avec `@react-pdf/renderer`
...
1. **Bilan hebdomadaire** (lundi 8 h, cron Vercel) — e-mail (Resend) **et** page
   `/bilan` : deadlines de la semaine, publications à programmer, validations client en
   retard, stats marquantes de la semaine passée. Ton bref, actionnable.
...
| Resend | bilans + rituels + envoi de rapports | `RESEND_API_KEY` |
...
1. E-mails Resend : domaine d'envoi (un domaine à toi ? sinon envoi vers ta boîte via
   l'adresse de test Resend au début).
2. Rapports PDF : logo Trinkets Design en pied de page (« préparé par ») ?
3. Moodboards : par marque, par thème, ou les deux ? (proposé : les deux)
4. Les 💡 de ce document : à confirmer ou retirer à la lecture.
